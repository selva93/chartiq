/*global CIQ, ChartLinker*/
'use strict';

var defaultGridSize = {cols:1,rows:1};
var defaultSymbols = ['MMM','AAPL','AXP','BA','CAT','CVX','CSCO','KO','DWDP','XOM','GS','HD','IBM','INTC','JNJ','JPM','MCD','MRK','MSFT','NKE','PFE','PG','TRV','UNH','UTX','VZ','V','WMT','WBA','DIS'];
...
function RemoveAllCharts(){
	...
	$('.chart-grid').children().not('#chart0').remove();
}
...
function AddCharts(cols, rows){
	...
	if( masterChart != null ){
		...
		if(rows == 1){
			...
			if(masterChart.stx.controls.floatDate) masterChart.stx.controls.floatDate.style.opacity=1;
		}else{
			masterChart.removeAttribute('x-axis');
			masterChart.stx.xaxisHeight = 0;
			if(masterChart.stx.controls.floatDate) masterChart.stx.controls.floatDate.style.opacity=0;
		}
	...
	if(localStorage){
		...
		localStorage.setItem('gridSize', JSON.stringify({cols:+cols, rows:+rows}));
	}
}
...
function SetChartSize(element, width, height, xaxis){
	...
	element.style.width = 'calc('+width+'vw - 10px)';
	element.style.height = 'calc('+height+'vh - '+(((height/100)*50)+margin)+'px)';
}
...
function AddChart(width, height, xaxis){
	...
	newInstantChart.setAttribute('symbol', defaultSymbols[chartGrid.childElementCount]);
	newInstantChart.setAttribute('tmpl-src', 'examples/templates/chart-grid/sample-template-grid-context.html');
	...
	newInstantChart.addEventListener('mouseup', function(event){
	...
	if(localStorage){
		...
		if ( chartLayout !== null && localStorage.getItem('myChartLayout'+newInstantChart.id) === null) {
			...
			localStorage.setItem('myChartLayout'+newInstantChart.id, JSON.stringify(tmpLayout));
		}
	}
...
function startUI(rootstxx){
	UIContext=new CIQ.UI.Context(rootstxx, $("#chart-grid[cq-context]"));
		...
		if(targetSymbol){
			...
			if(symbolData.name === activeMarket){
				...
				if(chartLinker.charts.size === 1){
					...
					alert("To set a chart symbol, first click on a chart to select.");
				}
			}else{
				alert('The symbol '+targetSymbol+' in market '+symbolData.name+' does not share the same hours as the current grid.');
			}
		}
	...
	// used to monitor keystrokes for symbol search in the global UI
	let KeystrokeHub=new CIQ.UI.KeystrokeHub($("body"), UIContext, {cb:CIQ.UI.KeystrokeHub.defaultHotKeys});
...
function restoreSettings(){
	...
	var layoutSetting=CIQ.localStorage.getItem('myChartLayout'+document.querySelector('cq-instant-chart').id);
	if(layoutSetting){
		layoutSetting = JSON.parse(layoutSetting);
		if(layoutSetting.crosshair) document.querySelector('cq-toggle[cq-member="crosshair"]').set(true);
	}

	var themeSetting=CIQ.localStorage.getItem('CIQ.Themes.prototype.current');
	if(themeSetting){
		themeSetting = JSON.parse(themeSetting);
		if(themeSetting.theme) changeTheme(themeSetting.theme);
	}
}

function changeTheme(className){
	// Update the page theme class before passing along to linker
	if(UIContext.currentLoadedBuiltIn) UIContext.topNode.classList.remove(UIContext.currentLoadedBuiltIn); 
	...
	CIQ.localStorage.setItem('CIQ.Themes.prototype.current',JSON.stringify({theme: className}));
}
...
CIQ.ChartEngine.prototype.prepend("mouseWheel",function(e){ 
	let targetInstantChart = $(e.currentTarget.stx.container).closest('cq-instant-chart').attr('id');
	if(targetInstantChart) chartLinker.setLinkerMaster(targetInstantChart);
	return false;
});

// Listen for instant charts ready
document.querySelector('body').addEventListener('signal-chart-ready', handleChartReady);
...
document.querySelector('body').addEventListener('update-grid', e => {
	...
	if(localStorage){
		...
		for( const [idx, componentRef] of document.querySelectorAll('cq-instant-chart').entries() ){
...
if(localStorage){
	...
	if(lsGridSize && typeof(lsGridSize.cols) === 'number' && typeof(lsGridSize.rows) === 'number') defaultGridSize = lsGridSize;
}
...
class LinkerFeatureInjector {
	...
	injectStudyUpdate() {
		$("cq-study-dialog").each(function(){
			this.updateStudy = function(updates){
				chartLinker.updateStudy(this.helper, updates);
			};
		});
	}
	...
	injectStudyMenuRenderer() {
		...
		$(".header cq-studies").each(function(){
	...
	injectStudyLegendRenderer(){
		...
		$(".header cq-study-legend").each(function(){
			// Overload the studies component render method to attach chart-linker to its events
