...
class ChartLinker {
	...
	remove(chartDetail) {
		...
		chartDetail.stx.removeInjection(chartDetail.injections.crosshair);
		chartDetail.stx.removeInjection(chartDetail.injections.scroll);

		chartDetail.stx.destroy();
		// Disable the instance resize handler because the global event is still holding on to it
		chartDetail.stx.resizeChart = chartDetail.stx.resizeCanvas = function(){return false;};

		this.charts.delete(chartDetail.stx);
		this.symbols.delete(chartDetail.symbol);
	...
	setCrosshairInjection({stx, injections, state}) {
		injections.crosshair = stx.prepend('positionCrosshairsAtPointer', () => {
			...
			if (!state.slave) {
				if(CIQ.withinElement(stx.chart.container, CIQ.ChartEngine.crosshairX, CIQ.ChartEngine.crosshairY)){
	...
	setCrosshair(master) {
		...
		const {left, cy, chart, chart: {yAxis: {height}}} = master;

		const cyPercentage = cy / height;
		const targetTick = master.tickFromPixel(CIQ.ChartEngine.crosshairX - left, chart);
		let date = 0;
		...
		if( targetTick >= 0 && targetTick < chart.dataSet.length ) date = master.dateFromTick(targetTick, chart, true);
	...
	addStudy(studyName) {
		...
		this.charts.forEach((details, stx)=>{
			...
			if(document.getElementById(details.id).hasAttribute('x-axis')){
				CIQ.Studies.studyLibrary[studyName].panelHeight = (stx.chart.canvasHeight*0.2)+20;
			}
	...
	UIHelperProxy(){
		...
		this.charts.forEach((details, stx)=>{
			UIContext.getAdvertised(helper)[method].apply(details,args);
		});
	}
	...
	getUIHelperProxy(){
		...
		// Extract the helper and method names so the rest can be passed as params to the named method
		let [helper, method] = args.splice(1,2);
		UIContext.getAdvertised(helper)['get'+method].apply(UILayout,args);
	}
	setUIHelperProxy(){
		let args = Array.prototype.slice.call(arguments);
		// Extract the helper and method names so the rest can be passed as params to the named method
		let [helper, method] = args.splice(1,2);
		this.charts.forEach((details, stx)=>{
			UIContext.getAdvertised(helper)['set'+method].apply(details,args);
		});
	}
}