...
(function (definition) {
		...
		function onConnected(data) {
					...
					if (key == "Errors") {
						...
						*/
						...
						cb({
							...
							moreAvailable: quoteData.length>=2 && (!params.endDate || quoteData[0].DT<params.endDate),
							attribution: {
								source: "fis_mm",
								exchange: delay
							}
			...
			try{
				...
				if(monitor){
						...
						if(!stx.isHistoricalMode()) {
							translateAndSendToChart(data, symbol, function(quotes) {
								...
								if(qts && qts.length) {
									if(params.series){
										for(var s in stx.chart.series){
											...
											if(ser.parameters.symbol==symbol){
												qts=stx.quoteDriver.cleanup(stx,ser,qts,CIQ.QuoteFeed.UPDATE,{symbol:symbol, chart:stx.chart});
											}
										}
									}else{
										qts=stx.quoteDriver.cleanup(stx,null,qts,CIQ.QuoteFeed.UPDATE,{symbol:symbol, chart:stx.chart});
									}
									...
									stx.createDataSet(null,null,{appending:true, appendToDate:qts[0].DT});
					...
					if(quoteFeedSunGard.debug || symbol=="IBIO") console.log(new Date()+" snapshot "+JSON.stringify(request));
					muse.snapshotCharts(JSON.stringify(request), function () {
						// make sure callback is executed only once
						var dup=false;
						return function(data){
							if(quoteFeedSunGard.debug || symbol=="IBIO") console.log(new Date()+(dup?" (dup)":"")+" response for "+reqID+": "+JSON.stringify(data));
							if(dup) return;
							dup=true;
							translateAndSendToChart(data, symbol, function(res){
								if(!params.series && res.quotes && res.quotes.length) stx.chart.attribution=res.attribution;
								cb(res);
							});
						};
					}());
	...
	quoteFeedSunGard.subscribe=function(params){
		if(params.stx.quoteDriver.getQuoteFeed(params).behavior.refreshInterval<=0) return;
