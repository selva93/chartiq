...
(function (definition) {
	...
	CIQ.QuoteFeed.Xignite.prototype.fetch=function(params, cb){
		...
		var isRate=CIQ.Market.Symbology.isRateSymbol(symbol) || _checkAssetClass(params.symbolObject.assetClass, CIQ.QuoteFeed.Xignite.Constants.RATES);
		...
		if(!params.symbol){
			...
			if(!isDaily){
				...
				if(match){
					...
					switch(match[3]){
					case "D": params.maturity+="Y"; break;
					case "M": params.maturity+="O"; break;
					case "Y": params.maturity+="R"; break;
					}
					symbol=match[1];
				}
		...
		if(api && args && error===""){
			...
			var mamaCallbackFunction=function(status, res){
				...
				function rateSort(a, b){
					...
					if(CIQ.strToDate(a.split(",")[i])<CIQ.strToDate(b.split(",")[i])) return -1;
					return 1;
				}
				...
				function processData(quotes, params){
					...
					if(api.func=="xRates" && api.results.maturity) arr=[arr[0]].concat(arr.slice(1).sort(rateSort));
					var dt=null;
					var stick=false;
					var NYOffsetMap={};
					for(i=1;i<arr.length;i++){
						...
						if(close>0){
							...
							if(!newQuotes[newQuotes.length-1] || +newQuotes[newQuotes.length-1].DT!==+bcdt){
							...
							if(api.func=="xRates" && api.results.maturity){
								if(!newQuotes[newQuotes.length-1].termStructure) newQuotes[newQuotes.length-1].termStructure={};
								var _yield=Number(fields[fieldNames[api.results.close]].replace("%",""));
	...
	CIQ.QuoteFeed.Xignite.Templates={
		...
		HistoricalRates: {
		    ...
		    statics: null,
		    fields: "Message,HistoricalInterestRate.Description.Maturity,HistoricalInterestRate.Rates.Date,HistoricalInterestRate.Rates.Text",
			results:{
				message:"Message",
