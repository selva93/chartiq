...
(function (definition) {
	...
	if (typeof exports === "object" && typeof module === "object") {
		...
		throw new Error("Only CommonJS, RequireJS, and <script> tags supported for quoteFeedForecastSimulator.js.");
	}
})(function(_exports){
	_exports = _exports || {};
	var CIQ = _exports.CIQ;

	var quoteFeedForecastSimulator=_exports.quoteFeedForecastSimulator={}; // the quotefeed object
	...
	quoteFeedForecastSimulator.certaintyPercentages=[0.35,0.15,0.08,0.05,0.02,0.01,0.01];  // these add up to 100% and help predict the Certainty matrix which can be plotted on a scatterplot.
	...
	quoteFeedForecastSimulator.generateData=function(stx, symbol, interval, period){
		...
		iterator.previous(quoteFeedForecastSimulator.periodsBack*stx.layout.periodicity);
		// look for first record before iterator point that has data
		var startTick;
		for(startTick=stx.chart.masterData.length-1;startTick>=0;startTick--){
			if(stx.chart.masterData[startTick].DT<=iterator.begin) break;
		}
		if(startTick<0) startTick=0;
		var originalStartValue=stx.chart.masterData[startTick], startValue, startDate, startVolume;
		...
		var factor=quoteFeedForecastSimulator.randomizationFactor, decimalPlaces=2;
		...
		var totalPeriods=(quoteFeedForecastSimulator.periodsBack+quoteFeedForecastSimulator.periodsForward)*stx.layout.periodicity;
		while(pushData.length<totalPeriods){
			...
			if(lastForecast){
				record={
					"DT":iterator.next(),
					"Close":Number(((Math.random()-0.5)*factor+lastForecast.Close).toFixed(decimalPlaces)),
					"Open":Number(lastForecast.Close),
					"Volume":lastForecast.Volume*(1+(Math.random()-0.5)/10),
					"Certainty":[],
				};
				record.High=record.Close*(1+quoteFeedForecastSimulator.spreadPercentage*pushData.length/totalPeriods);
				record.Low=record.Close*(1-quoteFeedForecastSimulator.spreadPercentage*pushData.length/totalPeriods);
				var certaintyPercentageLength=quoteFeedForecastSimulator.certaintyPercentages.length;
				for(var cp=0;cp<certaintyPercentageLength;cp++){
					...
					if(!cp) record.Certainty.push([record.Close,null,v]);
						...
						record.Certainty.push([record.Close+(cp/(certaintyPercentageLength-1))*(record.High-record.Close),null,v]);
						record.Certainty.push([record.Close+(cp/(certaintyPercentageLength-1))*(record.Low-record.Close),null,v]);
					}
				}
			}else{
				record={"DT":startDate, "Close":startValue, "High":startValue, "Low":startValue, "Open":startValue, "Volume":startVolume};
	...
	quoteFeedForecastSimulator.fetchInitialData=function (symbol, suggestedStartDate, suggestedEndDate, params, cb) {
		var quotes=quoteFeedForecastSimulator.generateData(params.stx, symbol, params.interval, params.period);
		if(!quotes) cb({error:"Not found"});
		else cb({quotes:quotes, moreAvailable:false, attribution:{source:"forecaster", exchange:"RANDOM"}});
	};

	// called by chart to fetch update data
	quoteFeedForecastSimulator.fetchUpdateData=function (symbol, startDate, params, cb) {
		var quotes=quoteFeedForecastSimulator.generateData(params.stx, symbol, params.interval, params.period);
		if(!quotes) cb({error:"Not found"});
		else cb({quotes:quotes, moreAvailable:false, attribution:{source:"forecaster", exchange:"RANDOM"}});
	};
	...
	quoteFeedForecastSimulator.fetchPaginationData=function (symbol, suggestedStartDate, endDate, params, cb) {
		var quotes=quoteFeedForecastSimulator.generateData(params.stx, symbol, params.interval, params.period);
		if(!quotes) cb({error:"Not found"});
		else cb({quotes:quotes, moreAvailable:false, attribution:{source:"forecaster", exchange:"RANDOM"}});
	};
